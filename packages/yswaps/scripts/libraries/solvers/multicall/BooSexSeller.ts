...
export class BooSexSeller implements Solver {
  ...
  async solve({
    ...
    console.log('[Boo] Total balance is', utils.formatEther(sexBalance), 'solid');
    ...
    const path = [[sex.address, wftm.address, false]];
    ...
    if (approveSex) {
      console.log('[Boo] Approving sex to solidly router');
    ...
    if (approveWftmSpooky) {
      console.log('[Boo] Approving wftm to spooky');
      const approveWftmTx = await sex.populateTransaction.approve(this.spookyRouter, constants.MaxUint256);
      await multicallSwapperSigner.sendTransaction(approveWftmTx);
      transactions.push(approveWftmTx);
    }

    console.log('[Boo] Executing sex => wftm');
    ...
    console.log('[Boo] Executing wftm => boo');
    ...
    console.log('[Boo] Final boo balance is', utils.formatEther(amountOut));
