...
export class CurveYfiEth2 extends BaseSolver implements Solver {
  ...
  protected async _loadContracts(): Promise<void> {
    this._crvYfiEth = await this._getTokenContract({
      tokenContractAddress: '0x29059568bB40344487d62f7450E78b8E6C74e0e5',
      signer: this._multicallSigner,
    });
    this._cvx = await this._getTokenContract({
      tokenContractAddress: '0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B',
      signer: this._multicallSigner,
    });
    this._crv = await this._getTokenContract({
      tokenContractAddress: '0xD533a949740bb3306d119CC777fa900bA034cd52',
      signer: this._multicallSigner,
    });
    this._weth = await this._getTokenContract({
      tokenContractAddress: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
      signer: this._multicallSigner,
    });
    this._crvSwap = await this._getContract<ICurveFi>({
      contractAddress: '0xC26b89A667578ec7b3f11b2F98d6Fd15C07C54ba',
      abi: ICurveFi__factory.abi,
      signer: this._multicallSigner,
    });
  }
  ...
  async solve({
    ...
    return await this._getExecuteTx({
      tradeFactory,
