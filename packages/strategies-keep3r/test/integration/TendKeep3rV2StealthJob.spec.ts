...
describe('TendKeep3rV2StealthJob', () => {
  ...
  let v2Keeper:any;
  let tendKeep3rV2StealthJob:any;
  ...
  it('Should deploy on mainnet fork', async function () {
    const TendKeep3rV2StealthJob = await ethers.getContractFactory('TendKeep3rV2StealthJob');

    tendKeep3rV2StealthJob = (
      await TendKeep3rV2StealthJob.deploy(
    ...
    // Add as valid job
    await v2Keeper.addJob(tendKeep3rV2StealthJob.address);
    ...
    await keep3rV2.addJob(tendKeep3rV2StealthJob.address);
    await keep3rV2.forceLiquidityCreditsToJob(tendKeep3rV2StealthJob.address, e18.mul(10000));
    ...
    await tendKeep3rV2StealthJob.setRewardMultiplier(950);

    for (const strategy of strategies) {
      await tendKeep3rV2StealthJob.addStrategy(
    ...
    const jobStrategies = await tendKeep3rV2StealthJob.strategies();
    ...
    await stealthRelayer.addJob(tendKeep3rV2StealthJob.address);
  ...
  it('Should work and pay in bonded KP3R', async function () {
    ...
    const rawTx = await tendKeep3rV2StealthJob.connect(keeper).populateTransaction.work(strategies[0].address);
    ...
    let workTx = await stealthRelayer.connect(keeper).execute(
      tendKeep3rV2StealthJob.address, // address _job,
    ...
    expect(await tendKeep3rV2StealthJob.callStatic.workable(strategies[0].address)).to.be.false;
  });

  it('Should change requirements', async function () {
    // Change req (bond == WETH)
    await tendKeep3rV2StealthJob.connect(owner).setKeep3rRequirements(
  ...
  it.skip('Should bond in WETH and pay in bonded KP3R', async function () {
    ...
    // populates work transaction
    const rawTx = await tendKeep3rV2StealthJob.connect(anotherKeeper).populateTransaction.work(strategies[0].address);
    const callData = rawTx.data;
