...
import * as gasprice from '../../../utils/arb-gas-price';
...
async function main() {
  ...
  console.log('[App] Using address', harvester.address, 'on arbitrumOne');
  ...
  for (let i = 0; i < strategies.length; i++) {
    ...
    try {
      if (!workable[i]) {
        ...
        console.log(`[App] Check work tx at https://arbiscan.io/tx/${tx.hash} sent ${moment()} (${moment().unix()})`);
      }
    } catch (error: any) {
